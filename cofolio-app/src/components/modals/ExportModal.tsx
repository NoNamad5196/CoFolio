...
export function ExportModal({ isOpen, onClose, portfolioName = '포트폴리오' }: Props) {
  ...
  useEffect(() => {
    if (isOpen) { setBusy(false); setDone(null) }
  }, [isOpen])
  ...
  const handleExport = async () => {
    ...
    if (format === 'html') setDone({ title: 'HTML 다운로드를 시작했어요', sub: `${portfolioName}-${theme}.html` })
    else if (format === 'pdf') setDone({ title: 'PDF 다운로드를 시작했어요', sub: `${portfolioName}-${theme}.pdf` })
    else if (format === 'md') setDone({ title: 'Markdown이 복사되었습니다', sub: 'GitHub 프로필 README에 붙여넣어 보세요' })
    else if (format === 'notion') setDone({ title: 'Notion 블록이 복사되었습니다', sub: 'Notion 페이지에 붙여넣으면 그대로 정리돼요' })
    else if (format === 'vercel') setDone({ title: 'Vercel 배포 패키지가 준비됐어요', sub: `vercel.com/import/${slug} — 한 번 클릭으로 배포` })
    setBusy(false)
  }
                ...
                {EXPORT_FORMATS.map((f) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className={cn('mt-0.5 h-4 w-4 shrink-0 rounded-full border', sel ? 'border-violet-400 bg-violet-400' : 'border-white/20')}>
                        {sel && (
              ...
              <>
                ...
                <PrimaryBtn size="sm" onClick={handleExport} disabled={busy}>
                  {busy ? (
                    ...
                    <>
                      {format === 'md' ? 'Markdown 복사' : format === 'notion' ? 'Notion으로 보내기' : format === 'vercel' ? '배포 패키지 만들기' : '다운로드'}
