// Lightweight inline SVG icon set (Lucide-style) so we don't depend on the lucide CDN.
const Ico = (props) => {
  const { name, size = 18, className = "", strokeWidth = 1.6, ...rest } = props;
  const common = {
  ...
  switch (name) {
    ...
    case "logo-c": return (<svg {...common} viewBox="0 0 24 24"><defs><linearGradient id="cg" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stopColor="#a78bfa"/><stop offset=".55" stopColor="#6366f1"/><stop offset="1" stopColor="#22d3ee"/></linearGradient></defs><circle cx="12" cy="12" r="9" stroke="url(#cg)" strokeWidth="2"/><path d="M16 8.5a5 5 0 1 0 0 7" stroke="url(#cg)" strokeWidth="2"/><circle cx="16.5" cy="12" r="1.6" fill="url(#cg)" stroke="none"/></svg>);
